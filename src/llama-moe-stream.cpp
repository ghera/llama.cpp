...
static void llama_moe_stream_remap_op(ggml_tensor * dst, const ggml_tensor * a, int ith, int nth, void * userdata) {
    ...
    if (ith != 0) {
        // worker: wait for thread 0 to publish this node's miss list, drain it.
        // one publication per node, fenced from the next by the ggml barrier
        static thread_local uint64_t seen = 0;
        auto & sync = *layer.stream->sync;
        while (sync.seq.load(std::memory_order_acquire) == seen) {
            std::this_thread::yield();
        }
        seen = sync.seq.load(std::memory_order_relaxed);
