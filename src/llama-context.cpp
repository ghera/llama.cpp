...
uint32_t llama_context::graph_max_nodes(uint32_t n_tokens) const {
    ...
    if (model.arch == LLM_ARCH_KIMI_K3) {
        ...
        // expert streaming builds wave-multiplied routed FFNs: any arch needs
        // the generous node budget while -smoe is active (gemma4 measured)
        model.moe_stream.enabled()) {
